-- KORA Payments P1-PAY-API-004B-1.
-- Expand response-key normalization without changing migration 034.
-- No endpoints or provider calls.
...
begin
  ...
  if jsonb_typeof(p_body) = 'object' then
    ...
    loop
      -- Normalize case and remove identifier separators so camelCase,
      -- snake_case and kebab-case aliases share the same comparison form.
      normalized_key := pg_catalog.regexp_replace(pg_catalog.lower(json_key), '[^a-z0-9]+', '', 'g');
