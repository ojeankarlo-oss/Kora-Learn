-- Resolve concurrent retries before validating state changed by the first call.
...
begin
  ...
  where provider = p_provider
    and provider_event_id = p_provider_payment_id || ':' || p_event_type
  for update;
  if found then
    ...
    then
      raise exception 'webhook event correlation conflict';
    end if;
